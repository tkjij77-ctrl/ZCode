...
import (
	...
	toolsPkg "github.com/tkjij77-ctrl/ZCode/internal/llm/tools"
...
func (c *copilotClient) exchangeGitHubToken(githubToken string) (string, error) {
	...
	req.Header.Set("User-Agent", "ZCode/1.0")
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in zcode.json, or ensure GitHub CLI/Copilot is properly authenticated.")
	...
	openaiClientOptions = append(openaiClientOptions,
		option.WithHeader("Editor-Version", "ZCode/1.0"),
		option.WithHeader("Editor-Plugin-Version", "ZCode/1.0"),
